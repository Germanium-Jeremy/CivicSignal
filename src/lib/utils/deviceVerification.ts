 ...
 */
export async function verifyDevice(userId: string, deviceInfo: DeviceInfo): Promise<{
  isVerified: boolean;
  reason?: string;
  ...
  try {
    ...
    const recentIssues = userIssues.filter(
      (issue: any) => 
        Date.now() - issue.submittedAt.getTime() < 24 * 60 * 60 * 1000 // Last 24 hours
    );
...
export async function checkSubmissionLimit(userId: string): Promise<{
  ...
  try {
    ...
    const MAX_DAILY_SUBMISSIONS = 10; // Maximum issues per day per user
