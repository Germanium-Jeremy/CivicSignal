...
export async function POST(request: NextRequest) {
     try {
          ...
          if (existingAgency) {
               return NextResponse.json({ error: "You have already registered an agency" }, { status: 400 });
          }
          ...
          if (error.message && error.message.includes("registration number already exists")) {
               return NextResponse.json({ error: "An agency with this registration number already exists" }, { status: 400 });
          }
