...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
     try {
          ...
          if (!categoryInfo || !agency.serviceDomains.includes(issue.category)) {
               return NextResponse.json({ error: "This issue is not within your agency's service domain" }, { status: 403 });
          }
          ...
          if (comment && comment.trim()) {
               ...
               updates.statusHistory.push({ status, comment: comment.trim(), changedAt: new Date(), changedBy: agency.name, });
          }

          const updatedIssue = await Issue.findByIdAndUpdate(id, updates, { new: true, runValidators: true }).populate("reportedBy", "fullName email").lean();
