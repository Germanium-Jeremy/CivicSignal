...
export default function AdminDashboardPage() {
     ...
     const fetchDashboardStats = async () => {
          try {
               ...
               if (dashboardResponse.success) {
                    ...
                    setTrends({
                         agencies: { value: Math.floor(Math.random() * 10) - 5, period: 'week' },
                         issues: { 
                         ...
                         users: { value: Math.floor(Math.random() * 20) - 10, period: 'week' }
                    });
               } else {
     ...
     const StatCard = ({ title, value, icon: Icon, color, subtitle, trend }: { title: string; value: number; icon: any; color: string; subtitle?: string; trend?: string; }) => (
     ...
     const DetailCard = ({ title, items }: { title: string; items: { label: string; value: number; color: string; icon: any }[] }) => (
               ...
               <div className="space-y-3">
                    {items.map((item, index) => (
                         <div key={index} className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-semibold text-almost-black">{item.value.toLocaleString()}</span>
                         </div>
                    ))}
               </div>
          </div>
          ...
          <AdminLayout>
               <div className="space-y-6">
                    ...
                    <div className="bg-linear-to-r from-accent2 to-accent rounded-xl p-6 text-white">
                         <h1 className="text-2xl font-bold mb-2"> Welcome back, {adminData?.fullName || 'Administrator'}!</h1>
                         <p className="text-white/90">Here's what's happening with CivicSignal today.</p>
                    </div>

                    {/* Main Stats */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                         <StatCard title="Total Agencies" value={stats.agencies.total} icon={FaBuilding} color="bg-blue-500" subtitle={`${stats.agencies.pending} pending approval`}
                         ...
                         <StatCard title="System Health" value={98} icon={FaChartLine} color="bg-purple-500" subtitle="Uptime percentage" trend="Excellent" />
                    </div>

                    {/* Detailed Stats */}
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                         <DetailCard title="Agency Status" items={[
                         ...
                         <DetailCard title="Issue Status" items={[
                         ...
                         <DetailCard title="User Distribution" items={[
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                         <div className="flex justify-between items-center mb-4">
                              ...
                              <Link href="/admin/issues" className="text-accent2 hover:text-accent2/80 text-sm font-medium flex items-center gap-1"> View All <FaEye className="text-xs" /> </Link>
                         </div>
                                        ...
                                        return (
                                             <div key={issue._id} className="flex items-center justify-between p-3 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors">
                                                  <div className="flex items-center gap-3 flex-1">
                                                       <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium" style={{ backgroundColor: `${categoryInfo.color}20`, color: categoryInfo.color }}>
                                                            {issue.trackingNumber ? issue.trackingNumber.split('-')[1] : 'N/A'}
                                                       </div>

                                                       <div className="flex-1 min-w-0">
                                                            <p className="text-sm font-medium text-almost-black truncate"> {issue.title || 'Untitled Issue'} </p>
                                                            <div className="flex items-center gap-2 text-xs text-gray-500">
                                                                 <span className="px-2 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: `${categoryInfo.color}20`, color: categoryInfo.color }}>
                                                                      {categoryInfo.name}
                                                                 </span>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              ...
                              <button onClick={() => window.location.href = '/admin/issues'}
                                   className="flex items-center gap-3 p-4 border-2 border-gray-200 rounded-lg hover:border-accent2 hover:bg-accent2/5 transition-all"
                              >
