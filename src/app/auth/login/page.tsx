...
export default function LoginPage() {
  ...
  const [verificationContacts, setVerificationContacts] = useState({
    email: "",
    phone: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        redirectAfterLogin(response.user?.role, response.isAdmin);
      }
    } catch (err: any) {
      ...
      if (
        ...
        err.message.includes("Account not fully verified")
      ) {
      ...
      if (
        ...
        errorMessage.includes("only available in Rwanda")
      ) {
        setError(
          "Access is only available from Rwanda. Please check your location.",
        );
  ...
  const handleSocialLogin = (provider: string) => {
    ...
    setError(
      "Social login is not yet implemented. Please use email and password.",
    );
  };
  ...
  const redirectAfterLogin = (role?: string, isAdmin?: boolean) => {
    ...
    if (normalizedRole === "citizen") {
      const citizenDeepLink = "civicsignal:///(tabs)/home";
      if (typeof window !== "undefined") {
        window.location.href = citizenDeepLink;
      }
      return;
    }
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="mt-3">
                    ...
                    <div className="space-y-1 text-xs">
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-2 h-2 rounded-full ${verificationStatus.emailVerified ? "bg-green-500" : "bg-red-500"}`}
                        ></span>
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-2 h-2 rounded-full ${verificationStatus.phoneVerified ? "bg-green-500" : "bg-red-500"}`}
                        ></span>
                        <span className="text-red-700">
                          Phone{" "}
        ...
        <div className="space-y-2">
          <label
            htmlFor="email"
            className="block text-sm font-medium text-almost-black"
          >
            Email Address
          </label>
          <input
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-4 bg-white text-neutral-text">
              Or continue with
            </span>
          </div>
        </div>
